...
## My code
<br><br>
<details> <summary> <b>First try (308 / 311 cases passed)<b> </summary>
    ...
    def threeSum(self, nums: List[int]) -> List[List[int]]:
        ...
        for i in range(n):
            for j in range(i + 1,n):
                if -(nums[i] + nums[j]) not in hashT:
                    hashT[-(nums[i] + nums[j])] = [[i,j]]
                else:
                    hashT[-(nums[i] + nums[j])].append([i,j])
        print(hashT)
        res = []
        for i,v in enumerate(nums):
            if v in hashT:
                for ele in hashT[v]:
                    if i not in ele and sorted([v,nums[ele[0]],nums[ele[1]]]) not in res:
                        res.append(sorted([v,nums[ele[0]],nums[ele[1]]]))
        return res
```
...
<br><br>
Inspired by [167. Two Sum II - Input Array Is Sorted](https://leetcode.cn/problems/two-sum-ii-input-array-is-sorted/solutions/), here is the answer
