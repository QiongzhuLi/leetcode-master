...
## [Leetcode Link](https://leetcode.cn/problems/queue-reconstruction-by-height/)
...
class Solution:
    def reconstructQueue(self, people: List[List[int]]) -> List[List[int]]:
        people = sorted(people,key=lambda x:x[0],reverse = True)  ## did not ascendingly sort the second element 
        print(people)
        res  = []
        for p in people:
            res.insert(p[1],p)
