...
## [Leetcode Link](https://leetcode.cn/problems/partition-labels/)
...
### Method 1 (Originally writen by myself, more effcient than method 2)
```python3
class Solution:
    def partitionLabels(self, s: str) -> List[int]:
        ...
        for i,v in enumerate(s):
            if v not in tmp:
                ...
                tmp[v][1] = i
        lol =[v for i,v in tmp.items()]
        lol.sort(key = lambda x:(x[0],x[1]))

        cur_end = -1
        i, res = 0, []
        while i < len(lol)-1:
            cur_end = max(cur_end,lol[i][1])
            if lol[i+1][0] > cur_end:
                res.append(lol[i+1][0])
            i += 1
        res = [0] + res + [len(s)]
        return [res[idx+1] - res[idx] for idx in range(len(res)-1)]
```
...
class Solution:
    def partitionLabels(self, s: str) -> List[int]:
        ...
        left, right ,res = 0, 0 , []
        for i,v in enumerate(s):
            right = max(right, hash[ord(v) - ord('a')])
            if i == right:
